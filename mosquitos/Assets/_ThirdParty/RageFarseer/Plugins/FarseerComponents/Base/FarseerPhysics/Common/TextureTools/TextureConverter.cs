...
namespace FarseerPhysics.Common
{
    ...
    public sealed class TextureConverter
    {
        ...
        public int AlphaTolerance
        {
            ...
            set { _alphaTolerance = (int)value << 24; }
        }
        ...
        public TextureConverter(Nullable<byte> alphaTolerance, Nullable<float> hullTolerance,
            Nullable<bool> holeDetection, Nullable<bool> multipartDetection, Nullable<bool> pixelOffsetOptimization, Nullable<FMatrix> transform)
        {
            Initialize(null, null, alphaTolerance, hullTolerance, holeDetection,
                multipartDetection, pixelOffsetOptimization, transform);
        }

        public TextureConverter(int[] data, int width)
        {
            Initialize(data, width, null, null, null, null, null, null);
        }

        public TextureConverter(int[] data, int width, Nullable<byte> alphaTolerance,
            Nullable<float> hullTolerance, Nullable<bool> holeDetection, Nullable<bool> multipartDetection,
            Nullable<bool> pixelOffsetOptimization, Nullable<FMatrix> transform)
        {
        ...
        private void Initialize(int[] data, Nullable<int> width, Nullable<byte> alphaTolerance,
            Nullable<float> hullTolerance, Nullable<bool> holeDetection, Nullable<bool> multipartDetection,
            Nullable<bool> pixelOffsetOptimization, Nullable<FMatrix> transform)
        {
        ...
        private void SetTextureData(int[] data, int width)
        {
            if (data == null)
		...
		/// <returns></returns>
		public static Vertices DetectVertices(int[] data, int width) {
        ...
        /// <returns></returns>
        public static Vertices DetectVertices(int[] data, int width, bool holeDetection)
        {
            TextureConverter tc =
        ...
        /// <returns></returns>
        public static List<Vertices> DetectVertices(int[] data, int width, float hullTolerance,
        ...
        public List<DetectedVertices> DetectVertices()
        {
            ...
            if (_data == null)
                throw new Exception(
                    "'_data' can't be null. You have to use SetTextureData(int[] data, int width) before calling this method.");

            if (_data.Length < 4)
                throw new Exception(
                    "'_data' length can't be less then 4. Your texture must be at least 2 x 2 pixels in size. " +
                    "You have to use SetTextureData(int[] data, int width) before calling this method.");
            ...
            if (_data.Length % _width != 0)
                throw new Exception(
                    "'_width' has an invalid value. You have to use SetTextureData(int[] data, int width) before calling this method.");
            ...
            Nullable<FVector2> holeEntrance = null;
            Nullable<FVector2> polygonEntrance = null;
        ...
        private Nullable<FVector2> SearchHoleEntrance(Vertices polygon, Nullable<FVector2> lastHoleEntrance)
        {
            if (polygon == null)
        ...
        private FVector2 GetTopMostVertex(Vertices vertices)
        {
            ...
            FVector2 topMost = new FVector2(0,0);
            ...
            if (entranceFound)
            {
                ...
                do {
                    ...
                    if (GetNextHullPoint(ref last, ref current, out next))
						hullArea.Add(next);									// Add the vertex to a hull pre vision list.
                    else
						break;												// Quit
        ...
        private bool SearchNextHullEntrance(List<DetectedVertices> detectedPolygons, FVector2 start, out Nullable<FVector2> entrance) {
            int x;
